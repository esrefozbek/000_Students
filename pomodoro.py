...
def pomodoro_dongusu(pomodoro_sayisi: int = 4, calisma_suresi: int = 25*60, kisa_mola: int = 5*60, uzun_mola: int = 15*60):
    for dongu in range(1, pomodoro_sayisi + 1):
        ...
        if dongu < pomodoro_sayisi:
            c.print(f"[cyan]⏳ Kısa mola başlıyor... ({kisa_mola // 60} dk)[/cyan]")
            devam = geri_say_bar(kisa_mola, "☕ Kısa Mola")
            if not devam:
                return
        else:
            c.print(f"[magenta]🎉 Tüm pomodorolar tamam! Şimdi uzun mola. ({uzun_mola // 60} dk)[/magenta]")
            geri_say_bar(uzun_mola, "🛌 Uzun Mola")

    c.print("[bold green]✅ Pomodoro seansı tamamlandı![/bold green]")

if __name__ == "__main__":
    # Daha hızlı test etmek için süreleri düşürebilirsin:
    pomodoro_dongusu(
        pomodoro_sayisi=4,
        calisma_suresi=1,  # gerçek kullanırken 25*60 yap
        kisa_mola=5,
        uzun_mola=10
    )
