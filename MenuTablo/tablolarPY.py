...
def tabloyaGonder(liste):
        if len(liste[0])==9:
                genel_TABLO_kriterli(liste)
        
        if len(liste[0])==8:
                genel_TABLO_kritersiz(liste)

altBox_stili=Menu.rastgele_box_stili()

def genel_TABLO_kriterli(liste: list):
    # Tablo yaratılıyor
 ...
 # Sol panel: tablo
    panel_sol = Panel(table, title="[bold cyan]Öğrenci Tablosu[/]", border_style="green", width=105)
...
def genel_TABLO_kritersiz(liste: list):
    # Tablo yaratılıyor
    titleBelow=f"[bold spring_green2]BULUNAN ÖĞRENCİLER TABLOSU[/]   [thistle1]box_stilim:[/]{altBox_stili[0]}"
    table = Table(title=titleBelow,
